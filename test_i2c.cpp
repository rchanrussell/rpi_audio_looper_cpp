...
//int main() {
  ...
  std::this_thread::sleep_for(std::chrono::milliseconds(100));
    ...
    for (int led = 0; led < 16; led++) {
      ...
      oi.SignalTrackRecording(led);
      std::this_thread::sleep_for(std::chrono::milliseconds(500));
      oi.SignalTrackPlayback(led);
      std::this_thread::sleep_for(std::chrono::milliseconds(500));
      oi.SignalInGroup(led);
      std::this_thread::sleep_for(std::chrono::milliseconds(500));

    }
    std::this_thread::sleep_for(std::chrono::milliseconds(3000));
    for (int led = 0; led < 16; led++) {
      oi.SignalTrackOff(led);
      oi.SignalNotInGroup(led);
      std::this_thread::sleep_for(std::chrono::milliseconds(500));
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(3000));
  ...
  while(1) {
    ...
    if (gi.ProcessAndHandleInputEvents()) {
      if (gi.LastEventWasForTrack()) {
	      std::cout << "Track:" << gi.GetLastTrack() << std::endl;
      } else {
	      std::cout << "Group:" << gi.GetLastGroup() << std::endl;
      }
#if 0
